...
2. The Separable Hamiltonian and $C\mat{X}$ DerivationTo avoid the full $\mat{C}$ matrix inversion, suppose the Hamiltonian can be separated into a "separable" part that can be expressed strictly in terms of these overlap matrices, and a smaller remainder:$$H = H^0 + H^R$$We define the action of the separable Hamiltonian $H^0$ on a basis function $\ket{g_e}$ such that it yields a linear combination of the basis function itself and its parameter derivatives:
