---
title: Born-Oppenheimer And Nonadiabaticity
---

## Overview

This section introduces the Born-Oppenheimer approximation, the origin of nonadiabatic coupling, diabatic representations, and the role of conical intersections in molecular quantum dynamics.

## Recommended Reading Path

1. [Born-Huang expansion](beginer/01_bornhuang_expansion.md)
2. [Adiabatic approximations](beginer/02_adiabatic_approximations.md)
3. [Diabatic representation](beginer/03_diabatic_representation.md)
4. [Introduction to conical intersections](beginer/04_conical_intersections.md)
5. [Symmetry of conical intersections](beginer/05_symmetry_of_conical_intersections.md)

## Intermediate Notes

- [Grouped Born approximations](intermediates/int01_group_born_approximations.md)
- [Conical intersections, connection to NACV and topology of conical intersections](intermediates/int02_conical_intersections.md)

## Derivations

- [Adiabatic TDSE using the Born-Huang expansion](derivations/derivation_adiabatic_tdse.md)
- [Dressed kinetic energy operator](derivations/derivation_dressed_term.md)
- [Anti-Hermitian property of the nonadiabatic coupling vector $\F$](derivations/derivations_antihermitian_F.md)
- [Off-diagonal Hellmann-Feynman property of the nonadiabatic coupling vector $\F$](derivations/derivation_offdiagonal_Hellman.md)
- [Vector convention for adiabatic and diabatic electronic bases](derivations/derivations_adiab_diab_convention.md)
- [Diabatisation](derivations/derivations_diabatisation.md)
- [Two-state conical-intersection demonstration](derivations/derivations_two_state_model.md)
- [Quadratic vibronic coupling model notation](derivations/derivations_QVC_model.md)
- [Sign of the $\pi$ closed-contour integral of $\F$](derivations/derivations_signconvention_closedloopf.md)
- [Diabatic linear terms](derivations/derivations_diabatic_linearterms.md)

## Advanced Notes

- [Exact factorisation](advanced/adv01_exactfactorisation.md)

## Visualisations

- [QVC conical-intersection visualiser](visuals/different_conical_intersections.md)
