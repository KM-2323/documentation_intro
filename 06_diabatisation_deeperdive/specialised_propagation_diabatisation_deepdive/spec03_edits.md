...
The adiabatic-to-diabatic transformation (ADT) is introduced ([diabatic representation](../../02_Born_Oppenheimer_and_Nonadiabaticity/beginer/03_diabatic_representation.md) and [propagation diabatisation](../intermediates/int01_propagation_diabatisation_main.md) ) as the transformation that removes the first-order nonadiabatic coupling vectors from the nuclear kinetic-energy operator. Locally, this idea is expressed by the differential equation
...
Since \(\mat R\) is generally a multidimensional nuclear coordinate vector, Eq. \(\eqref{eq:adt_overview_equation}\) represents a system of coupled first-order partial differential equations. Written componentwise,
...
where \(\mat F_\alpha\) is the coordinate-resolved nonadiabatic coupling matrix along the nuclear coordinate \(R_\alpha\).

Contrasting to its single-dimension counterpart, along a single path one can usually integrate the equation if the coupling is finite along it. But defining a unique, smooth, path-independent transformation matrix over a whole region of nuclear configuration space is a stronger requirement.

This page gives the conceptual overview. The detailed derivations and explorations are deferred to the linked derivation and theory pages. The main questions are:
...
which is the non-Abelian character of the multistate ADT problem.
